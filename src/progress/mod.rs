...
/// Creates a new progress bar with the given length, managed by the global
/// `MultiProgress` instance, and applies the default progress style.
///
/// # Arguments
/// - `length`: The total length of the progress bar (e.g., 100 for percentage-based progress).
///
/// # Returns
/// - A `ProgressBar` instance managed by `global_multi_progress`.
pub fn new_progress_bar(length: u64) -> ProgressBar {
    let pb = global_multi_progress().add(ProgressBar::new(length));
